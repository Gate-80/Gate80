...
"""
RASD - Step 3: Extract Behavioral Features
...
Features computed per session:
  --- Session metrics ---
  1.  total_requests          total number of requests in session
  2.  session_duration_sec    time from first to last request (seconds)
  3.  requests_per_minute     request rate

  --- Error signals ---
  4.  failed_login_count      number of failed login attempts
  5.  error_ratio             proportion of non-200 responses
  6.  error_4xx_count         count of 4xx responses
  7.  error_5xx_count         count of 5xx responses

  --- Endpoint behavior ---
  8.  unique_endpoints        number of distinct paths hit
  9.  endpoint_entropy        Shannon entropy of endpoint distribution
  10. has_admin_access        1 if any admin endpoint was hit
  11. admin_action_count      number of admin requests

  --- Financial behavior ---
  12. wallet_action_count     number of wallet operations
  13. wallet_action_ratio     wallet actions / total actions
  14. transfer_count          number of transfer actions
  15. topup_count             number of topup actions
  16. withdraw_count          number of withdraw actions
  17. pay_bill_count          number of pay-bill actions
  18. financial_error_count   wallet actions that returned 400 (overdraft etc)

  --- Timing behavior ---
  19. avg_think_time_ms       average think time between actions
  20. std_think_time_ms       standard deviation of think time
  21. avg_response_time_ms    average server response time
  22. min_think_time_ms       minimum think time (bot detection)

  --- Identity ---
  23. persona                 assigned persona label
  24. geo_location            user location
  25. client_type             web / ios / android
  26. user_id                 user identifier
  27. email                   user email
...
print(f"      {len(session_meta)} session objects loaded.")
...
for session_id, requests in sessions_requests.items():
    ...
    admin_action_count = sum(1 for r in requests if r["action"] in ADMIN_ACTIONS
    ...
    avg_response_time_ms = min(safe_mean(response_times), 500.0)  # cap outliers from 500 errors
...
print(f"\n✅ Done! {len(output_rows)} session rows written to {OUTPUT_FILE}")
