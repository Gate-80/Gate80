...
from backend_api.db.audit_helper import (
    log_admin_view_users,
    log_admin_view_wallets,
    log_admin_view_transactions
)
...
def admin_view_all_users(
    ...
    log_admin_view_users(
        db=db, 
        admin_id=admin.id, 
        filters={"q": q, "verified": verified, "limit": limit, "offset": offset}
    )
...
def admin_view_all_wallets(
    ...
    log_admin_view_wallets(
        db=db,
        admin_id=admin.id,
        filters={"currency": currency, "status": status, "user_id": user_id, "limit": limit, "offset": offset}
    )
...
def admin_view_all_transactions(
    ...
    # Note: Filtering by currency requires JSON field query (more complex)
    # Skipping for simplicity, can be added later if needed
...
def admin_view_system_financial_overview(
    ...
    for c in currencies:
        ...
        # Calculate volume of completed transactions
